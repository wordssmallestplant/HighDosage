# HighDosage
My final project for web design, a coffee based html document set!!

The ZIP file contains the following:
> 2 css folders, one for general styling and one for styling the latte game.
> 2 js folders, one for general functionality and one for the latte game (again, lol).
> 6 html files, (index, farming, history, flavors, coffeematch, createalatte).
> 3 image folders for added organization (!these are in a css subfolder!).
> Audio folder containing audio for the latte game
> This is for if you want to run it *separately* from github on your own device OR use the assets.

> 4/6 pages have javascript functionality, the only ones without are farming + history as they're mainly infographic.
The entire project has been throughly tested and debugged - no errors show in console.log() upon booting. 

> All drawn and recorded assets are handmade by yours truly ~ while not very good, are entirely free to use!

> For further inquiries or to inform of bugs, contact me at: immon003@crk.umn.edu



